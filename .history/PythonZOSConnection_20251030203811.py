...
Surface=[SysLDE.GetSurfaceAt(i) for i in range(1, num_surfaces - 1)] # Use a list to store all surfaces objects
...
Paraxial = ZOSAPI.Editors.LDE.SurfaceType.Paraxial

for i in range(2, num_surfaces - 1):
    s = SysLDE.GetSurfaceAt(i).GetSurfaceTypeSettings(Paraxial)
    Surface_Type.append(s)

for i in range(2, num_surfaces - 1):
    SysLDE.GetSurfaceAt(i).ChangeType(Surface_Type[i])